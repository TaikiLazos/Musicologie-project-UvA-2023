...
ggplot(songs, aes(track.duration_min)) + geom_histogram() + facet_wrap(~ playlist_name)
```
...
ggplot(donut_df, aes(x=key_name, y=n, fill=key_name)) +
  geom_bar(stat="identity")+facet_grid(~ playlist_name)
```
...
ggplot(songs, aes(valence, tempo, color = key_name)) + geom_point() + facet_wrap(~ playlist_name) 
```
